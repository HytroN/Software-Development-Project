...
public class AdjacencyGraph {
    ...
    public void k() {
        ...
        ArrayList<String> VerticesSurplasNames = new ArrayList<>();
        ArrayList<Integer> VerticesSurplas = new ArrayList<>();
        ...
        Vertices.forEach(vertex -> {
            ...
            if (diff > 0) {
                VerticesSurplasNames.add(vertex.name);
                VerticesSurplas.add(diff);
            } else {
        ...
        while (true) {
            ...
            if (i < VerticesSurplas.size()) {
                String VSName = VerticesSurplasNames.get(i);
                String VLName = VerticesLackNames.get(j);

                int VS = VerticesSurplas.get(i);
                int VL = VerticesLack.get(j);

                if (VS >= (-VL)) {

                    VerticesSurplas.set(i, VS - (-VL));
                    VerticesLack.set(j, 0);

                    System.out.println(VSName + " sent " + (-VL) + " comtainer to " + VLName);
                    j++;

                    if (i == VerticesSurplas.size() - 1 && VerticesSurplas.get(i) == 0) break;

                } else {
                    VerticesLack.set(j, VL + VS);
                    VerticesSurplas.set(i, 0);
    ...
    public int getFlowPrice() { // denne funktion returne den samlede pris for at returne skibene til de havene som den kom fra og på samme måde som da man sendte den
